/*******************************************************************************************
1013. Partition Array Into Three Parts With Equal Sum
Easy

Given an array A of integers, return true if and only if we can partition the array 
into three non-empty parts with equal sums.

Formally, we can partition the array if we can find indexes i+1 < j with 
(A[0] + A[1] + ... + A[i] == A[i+1] + A[i+2] + ... + A[j-1] == A[j] + A[j-1] + ... + A[A.length - 1])


Example 1:

Input: [0,2,1,-6,6,-7,9,1,2,0,1]
Output: true
Explanation: 0 + 2 + 1 = -6 + 6 - 7 + 9 + 1 = 2 + 0 + 1

Example 2:

Input: [0,2,1,-6,6,7,9,-1,2,0,1]
Output: false

Example 3:

Input: [3,3,6,5,-2,2,5,1,-9,4]
Output: true
Explanation: 3 + 3 = 6 = 5 - 2 + 2 + 5 + 1 - 9 + 4

 

Note:

    3 <= A.length <= 50000
    -10000 <= A[i] <= 10000

*******************************************************************************************/

class Solution {
public:
    bool canThreePartsEqualSum(vector<int>& A) {
        if(A.size() < 3) return false;
        
        for(int i=1; i<A.size(); i++) A[i] += A[i-1];
        int total = A.back();
        if(total%3 != 0) return false;
        
        auto lft = find(A.begin(), A.end(), total/3);
        if(lft == A.end()) return false;
        
        auto rgt = find(lft, A.end(), total/3*2);
        return rgt != A.end();
        
    }
    
    
    bool canThreePartsEqualSum2(vector<int>& A) {
        if(A.size() < 3) return false;
        
        int total = std::accumulate(A.begin(), A.end(), 0);
        if (total%3 != 0) return false;
        
        int left = 1, right = A.size()-2;
        int s=A[0];
        while(left < A.size() && s != total/3){ s += A[left], left++;};
        if(left > A.size()-2) return false;
        
        s = A.back();
        while(right > left && s != total/3) {s += A[right], right--;}
        return s == total/3;
        
    }
};